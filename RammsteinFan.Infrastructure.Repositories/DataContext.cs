...
namespace RammsteinFan.Infrastructure.Repositories
{
    public class DataContext:DbContext
    {
        ...
        {
            modelBuilder.Entity<User>().Property(u => u.FavoriteAlbum).HasDefaultValue("No favorite");
            modelBuilder.Entity<User>().Property(u => u.FavoriteSong).HasDefaultValue("No favorite");
            ...
            modelBuilder.Entity<User>().HasData(new User[]
            {
                new User()
                {
                    Id = 1,
                    Name = "Ramil",
                    EmailAdress = "Chico5Listo4@gmail.com",
                    Age = 19,
                    RoleId = 2,
                    Password = "12"
                }
            ...
            modelBuilder.Entity<Replica>().HasData(new Replica[]
            {
                new Replica("Данил","Это Тиль",1){Id=1, Comments=1},
                new Replica("Данил","Нет, это Рихард",1, 4){Id = 2 },
                new Replica("Данил","Это Пауль",1){Id = 3 }
            });

            modelBuilder.Entity<DiscussionSubject>().HasData(new DiscussionSubject[]
            {
...
"Левой! Два,(3x) три, четыре! (2x)\n"){Id = 5 },
            new Content("Links 2 3 4","SongDescription","SongTranslationsMain/Links 2 3 4",
                ...
                "то, что ХОТЯТ видеть, не вникая в то, как обстоят дела на самом деле. Они не ЗНАЮТ, они ДУМАЮТ, что знают - прим. Max'a"){Id = 6 },
